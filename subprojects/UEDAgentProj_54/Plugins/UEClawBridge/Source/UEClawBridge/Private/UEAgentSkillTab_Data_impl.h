...
static const TCHAR* SkillRefreshPyScript = TEXT(
	...
	"\n"
	"def _find_source(n):\n"
	"    \"\"\"在 _source_skill_map 中查找，自动尝试连字符/下划线变体\"\"\"\n"
	"    if n in _source_skill_map: return _source_skill_map[n]\n"
	"    h = n.replace('_', '-')\n"
	"    if h in _source_skill_map: return _source_skill_map[h]\n"
	"    u = n.replace('-', '_')\n"
	"    if u in _source_skill_map: return _source_skill_map[u]\n"
	"    return {}\n"
	"\n"
	...
	"        # 从项目源码提升 layer（自动匹配连字符/下划线变体）\n"
	"        _src_info = _find_source(cn) or _find_source(m.name)\n"
	"        if _src_info:\n"
	"            _layer = _src_info.get('layer', _layer)\n"
	...
	"                        desc = _fl[:120]; break\n"
	"        except: pass\n"
	"        _src_info = _find_source(cn) or _find_source(name)\n"
