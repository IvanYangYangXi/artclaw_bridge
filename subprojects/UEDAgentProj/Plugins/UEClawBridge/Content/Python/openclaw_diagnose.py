"""
openclaw_diagnose.py - OpenClaw Gateway 连接诊断工具
=====================================================
职责: 逐项检查 Gateway 连接所需条件并生成诊断报告。
"""
# Ref: docs/UEClawBridge/troubleshooting/OpenClaw通信排错记录.md
...
def diagnose_connection(gateway_url: str = "", token: str = "") -> str:
    """
    诊断 OpenClaw Gateway 连接，逐项检查并生成报告。
    Returns: 多行诊断报告文本
    """
    gw = _load_gateway_config()
    url = gateway_url or f"ws://127.0.0.1:{gw.get('port', 18789)}"
    tok = token or gw.get("auth", {}).get("token", _DEFAULT_TOKEN)
    ...
    if errors > 0:
        ...
        try:
            results = asyncio.run(_test_handshake(url, tok))
            for line in results:
                lines.append(f"  {line}")
...
async def _test_handshake(url: str, token: str) -> list[str]:
    """实际执行握手测试"""
    import websockets
    results: list[str] = []
    try:
        async with websockets.connect(url, open_timeout=5.0) as ws:
            ...
            if msg.get("event") == "connect.challenge":
                ...
                results.append(f"⚠️  期望 connect.challenge，收到: {msg.get('event', '?')}")
                return results
            ...
            await ws.send(json.dumps({
                ...
                "params": {
                    ...
                    "client": {"id": _CLIENT_NAME, "displayName": "UE Agent Diagnose",
