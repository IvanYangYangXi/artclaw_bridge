...
void SArtClawToolPanel::OnOpenFullManager()
{
    FString Url = FString::Printf(
        TEXT("http://localhost:9876/chat?dcc=ue5&version=%s&session=%s"),
...
| 参数 | 说明 | 示例 |
|------|------|------|
| `dcc` | DCC 类型和版本 | `ue5`, `maya2024`, `comfyui` |
...
**UE**:
```
http://localhost:9876/chat?dcc=ue5&version=5.4.0&session=abc-123&level=MainLevel&selected=Cube,Sphere&selected_count=2
```
...
interface DCCContext {
  dccType: 'ue5' | 'maya2024' | 'comfyui' | 'sd' | 'sp';
  version: string;
  sessionId: string;
  context: {
    // UE 特有
