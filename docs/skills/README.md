...
│
└── ~/.openclaw/workspace/skills/               # 统一安装目录（运行时，扁平结构）
...
**注意**: Skills 现已统一安装到外部目录（`~/.openclaw/workspace/skills/` 或按平台配置），不再存储在 UE 插件内部的 `Content/Python/Skills/`。
...
以下官方 Skill 是最佳参考（现已统一安装到 `~/.openclaw/workspace/skills/`）：
