...
> **v1.3 更新**：MCP 工具精简重构——每个 DCC 只保留 1 个 MCP 工具，其余能力降级为 Python API。

必须支持 OpenClaw 调用以下 MCP 工具：

- `run_ue_python` —— UE 侧唯一 MCP 工具，执行任意 UE Python 代码
- `run_python` —— DCC 侧（Maya / Max 等）唯一 MCP 工具，执行任意 DCC Python 代码

原有的 `focus_on_actor`、`highlight_actors` 及后续 scene / asset / material / render 等能力**不再注册为 MCP 工具**，降级为 Python API，AI 通过 `run_ue_python` / `run_python` 执行代码调用。
...
- [ ] `run_ue_python` / `run_python` 的参数 Schema 清晰（核心参数：`code: str`）
- [ ] 返回值结构统一
- [ ] 错误信息可读、可回传
...
> **v1.3 更新**：Skill 不再注册为 MCP 工具，但 Skill Hub 仍然是 Python 模块的统一管理中心。AI 通过 `run_ue_python` / `run_python` 调用 `skill_hub.execute_skill()` 来执行 Skill。

**要求**：
- [ ] OpenClaw 不维护独立 Skill 清单
- [ ] Skill 元数据以 UE 插件侧为唯一事实来源
- [ ] 参数校验、异常格式化、日志记录统一处理
- [ ] Skill Hub 作为 Python API 层统一管理所有 Skill 的注册、发现与执行
...
**版本**：1.3 | **更新**：2026-03-27 | **状态**：PRD / Checklist 版