...
| 平台 | 适配器文件 | 接入方式 | 接入模式 | 状态 |
|------|-----------|---------|---------|------|
| **OpenClaw** | `platforms/openclaw/openclaw_adapter.py` | WebSocket Gateway | **全功能** | ✅ 完整实现 |
| **LobsterAI** | `platforms/lobster/lobster_adapter.py` | WebSocket Gateway | **全功能** | ✅ 完整实现 |
| **Claude Code** | `platforms/claudecode/claudecode_adapter.py` | stdio→MCP 桥接 | MCP-only | 🟡 POC |
| **Cursor** | `platforms/cursor/cursor_adapter.py` | stdio→MCP 桥接 | MCP-only | 🟡 POC |
| **WorkBuddy** | `platforms/workbuddy/workbuddy_adapter.py` | stdio→MCP 桥接 | MCP-only | 🟡 POC |

**接入模式说明**：

- **全功能**：支持 DCC 内嵌聊天面板 + 工具调用 + Skill 上下文注入 + Agent 切换 + 会话管理
- **MCP-only**：仅支持工具调用。MCP 协议是工具调用协议，不支持双向聊天、Skill 上下文注入、DCC 内嵌面板等功能。平台使用自己的 AI 模型，DCC 端聊天面板无法与之通信。

> MCP-only 平台适合已在 IDE/终端中工作、顺手调用 DCC 工具的轻量场景。
> 如需完整的 AI + DCC 交互体验，建议使用 **OpenClaw** 或 **LobsterAI**。
