...
| **用户创建 Skill** | 放 ArtClaw `02_user/` 层，注册为 MCP | 默认放 `~/.openclaw/workspace/skills/`，想共享再推到市集 |
...
artclaw skill install <skill-name>
  ├── 1. 下载/复制 Skill 包
  ├── 2. 将 SKILL.md → ~/.openclaw/workspace/skills/<skill-name>/SKILL.md
  ├── 3. 将 Python 库文件 → DCC 的 Python 路径（如果有）
  └── 4. 注册到 OpenClaw 的 skill 列表
```

**install.bat** 也需要更新：在部署 DCC 插件的同时，将 OpenClaw Skill 文件复制到 `~/.openclaw/workspace/skills/`。
...
| `docs/specs/项目目录结构说明.md` | 新增 `~/.openclaw/workspace/skills/` 目录说明 |
...
1. 在 `~/.openclaw/workspace/skills/` 下创建 5 个 Skill 目录
...
AI 通过 run_python 调用:
  ...
  # 3. 热加载注册
    │
    ▼
同时生成 OpenClaw SKILL.md → 保存到 ~/.openclaw/workspace/skills/batch-rename/
    │
    ▼
