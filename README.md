...
ArtClaw 使用统一 Skill 管理系统，Skills 安装到外部配置目录，可跨 DCC 软件复用：

### Skill 目录结构

```
项目源码（开发）:
├── skills/official/         # 官方 Skill 源码
│   ├── universal/           # 通用 Skills
│   ├── unreal/             # UE 专用 Skills  
│   ├── maya/               # Maya 专用 Skills
│   └── max/                # Max 专用 Skills
├── skills/marketplace/      # 市集 Skill 源码
└── team_skills/            # 团队共享 Skill

已安装目录（运行时）:
~/.openclaw/skills/         # 统一安装目录（扁平结构）
...
**安装流程**: `install.py --openclaw` 扫描项目源码，用 `shutil.copytree` 复制整个 Skill 目录（含代码和文档）到统一安装目录。`skill_hub.py` 通过 `~/.artclaw/config.json` 的 `skills.installed_path` 读取安装路径。
...
### 有MCP和Skill就可以接入大模型了，为什么要做这个桥接工程？

目的是优化使用者体验，就像VScode有很多agent插件，让用户能在原本软件的窗口里使用能大大提高使用的意愿和使用效率，并且可以根据需求做一些定制开发。

### 生产落地的思考

一些简单任务，比如按明确规则批量生成一些对象，通过MCP接入后，直接就能干活了。对于性能优化等分析工作、脚本开发等可以通过代码执行的任务也完全能直接胜任。但是这些应用场景大多是TA和程序的工作，完全无法帮到美术同学。现在的好处是，美术可以直接让AI帮做一些简单脚本能实现的功能，而不需要去学习编程，美术们自己也能做一些原本要TA支持的脚本。但是美术同学本身的工作内容AI完全无法直接上手完成。

大模型直接执行的过程是个黑盒，完全不知道内部是怎么工作的，AI 的执行结果完全无法预测，无法可控执行过程，就像最早期的AI生图，AI能画图，但是无法在项目落地，后面出了很多工程化的工具，让AI的执行过程更加可控，才能真正提升生产效率。

所以我们接下来需要做的是把过程拆解，让AI的产出变得可控，而这个过程还是需要依靠传统的工程化思维，claude code的代码也验证了这个方向是对的，他们没有很多黑科技，而是通过工程化让大模型以正确可控的方式去执行。
