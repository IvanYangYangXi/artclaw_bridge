"""
artclaw_bridge.config - 路径发现与配置
=================================

自动检测项目结构，定位:
  - artclaw_bridge/skills/ (官方库)
  - team_skills/ (团队库)
  - ~/.artclaw_bridge/skills/ (用户库)
  - UEClawBridge/Content/Python/Skills/ (运行时目录)
  - artclaw_bridge/skills/templates/ (模板目录)
"""
...
def find_project_root(start: Optional[Path] = None) -> Optional[Path]:
    """
    向上搜索项目根目录（包含 artclaw_bridge/ 目录或 .artclaw_bridge_root 标记文件的目录）。
    """
    current = start or Path.cwd()
    for parent in [current] + list(current.parents):
        if (parent / "skills").is_dir() and (parent / "docs").is_dir():
            return parent
        if (parent / ".artclaw_bridge_root").exists():
            return parent
        if (parent / "artclaw_bridge").is_dir() and (parent / "artclaw_bridge" / "skills").is_dir():
            return parent / "artclaw_bridge" if (parent / "artclaw_bridge" / "skills").is_dir() else parent
    return None
...
def get_runtime_skills_dir(project_root: Optional[Path] = None) -> Optional[Path]:
    ...
    # 常见路径
    candidates = [
        root / "subprojects" / "UEDAgentProj" / "Plugins" / "UEClawBridge" / "Content" / "Python" / "Skills",
        root / "Plugins" / "UEClawBridge" / "Content" / "Python" / "Skills",
    ]
    for c in candidates:
        if c.is_dir():
            return c
...
def get_user_skills_dir() -> Path:
    """获取用户个人 Skill 目录 (~/.artclaw_bridge/skills/)"""
    user_dir = Path.home() / ".artclaw_bridge" / "skills"
