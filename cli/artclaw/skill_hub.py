"""
artclaw.skill_hub - CLI 侧 Skill 管理中心
=============================================

实现 Skill 的分层加载、版本匹配和冲突检测。

加载优先级（数字越小优先级越高）:
  - 00_official/  ← artclaw/skills/     官方库
  - 01_team/      ← artclaw/team_skills/ 团队库
  - 02_user/      ← ~/.artclaw/skills/  用户库
...
logger = logging.getLogger("artclaw.skill_hub")
...
class SkillHub:
    ...
    def __init__(self, config: Optional[ArtClawConfig] = None) -> None:
        """初始化 SkillHub。

        Args:
            config: ArtClawConfig 实例。为 None 时自动创建。
        """
        self._config = config or ArtClawConfig()
