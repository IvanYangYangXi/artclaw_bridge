"""
claudecode_adapter.py — Claude Code 平台适配器（MCP-only）
==========================================================
实现 PlatformAdapter 接口。

Claude Code 是 CLI 模式的 AI 编程助手，通过 stdio→WebSocket 桥接器
（artclaw_stdio_bridge.py）与 DCC MCP Server 通信。
聊天在 Claude Code 终端中进行，本端仅负责 MCP 工具调用桥接。

与 Claude Desktop 的区别：
  - 配置文件路径不同：~/.claude.json（全局）或 .mcp.json（项目级）
  - 支持 `claude mcp add` CLI 命令进行配置
  - 本质上都是 MCP-only 模式
...
class ClaudeCodeAdapter(PlatformAdapter):
    ...
    与 Claude Desktop 的区别：
    - 配置路径：~/.claude.json（全局）或 .mcp.json（项目级）
    - 配置方式：支持 `claude mcp add` CLI 命令
    - 运行环境：终端 CLI，非桌面应用
    """
    ...
    def connect(self, gateway_url: str = "", token: str = "", **kwargs) -> bool:
        ...
        Claude Code 模式下，"连接" 指 DCC MCP 端口是否已监听。
        此处做 TCP socket 探测。
        """
        host = self._mcp_host
        port = self._mcp_port
    ...
    def send_message(self, message: str, timeout: float = 1800.0) -> str:
        """
        Claude Code 模式下不支持消息发送。

        聊天在 Claude Code 终端中进行，本适配器仅负责 MCP 工具桥接。

        :raises NotImplementedError: Claude Code 模式下不支持消息发送
        """
        raise NotImplementedError(
            "ClaudeCodeAdapter 不支持 send_message()。\n"
            "Claude Code 模式下，消息发送由 Claude Code CLI 负责。"
        )
    ...
    def send_message_async(self, message: str, callback: Callable[[str], None]) -> None:
        ...
        """
        raise NotImplementedError(
            "ClaudeCodeAdapter 不支持 send_message_async()。原因同 send_message()。"
        )
    ...
    def get_agent_id(self) -> str:
        """Claude Code 无多 Agent 概念，返回固定标识。"""
        return "claudecode"

    def list_agents(self) -> list:
        """Claude Code 无多 Agent 概念，返回空列表。"""
        return []

    def set_agent(self, agent_id: str) -> None:
        """Claude Code 不支持多 Agent 切换。"""
        pass
    ...
    def diagnose_connection(self, gateway_url: str = "") -> str:
        """
        诊断 Claude Code MCP 桥接连接状态。

        检查：MCP 端口可达性、stdio 桥接脚本、Claude Code 配置文件。
        """
        lines = ["=" * 60, "  Claude Code MCP 桥接诊断", "=" * 60]
        ...
        # [3/3] Claude Code 配置
        lines.append("\n[3/3] Claude Code 配置...")
        config_paths = self._find_claude_code_configs()
        if config_paths:
            ...
            lines.append("  ℹ️  Claude Code 配置文件未找到")
            lines.append("     全局配置: ~/.claude.json")
            lines.append("     项目配置: .mcp.json（项目根目录）")
            lines.append("     或运行: claude mcp add artclaw-ue -- python artclaw_stdio_bridge.py --port 8080")
    ...
    def _find_claude_code_configs(self) -> list:
        """
        查找 Claude Code 配置文件。
        返回 [(label, path), ...] 列表。
        """
        ...
        # 全局配置：~/.claude.json
        global_config = os.path.join(home, ".claude.json")
        if os.path.exists(global_config):
            found.append(("全局配置 (~/.claude.json)", global_config))

        # 项目级配置：.mcp.json（当前工作目录）
        cwd_config = os.path.join(os.getcwd(), ".mcp.json")
        if os.path.exists(cwd_config):
            found.append(("项目配置 (.mcp.json)", cwd_config))

        return found

    def __repr__(self) -> str:
        return (
            f"<ClaudeCodeAdapter platform={self.platform_type!r} "
